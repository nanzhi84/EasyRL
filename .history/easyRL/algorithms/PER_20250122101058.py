...
class PER_DDQN(DDQN):
    ...
    def learn(self, done):
        ...
        batch_reward = torch.FloatTensor(np.array([m[2] for m in batch_memory]))
        batch_next_state = torch.FloatTensor(np.array([m[3] for m in batch_memory]))
        
        # DDQN计算目标Q值
        q_eval_next = self.eval_net(batch_next_state).detach()
        eval_act_next = q_eval_next.max(dim=1)[1].unsqueeze(1)
        q_next = self.target_net(batch_next_state).detach().gather(1, eval_act_next).squeeze()
        q_target = batch_reward + (1 - done) * self.gamma * q_next
        ...
        for i in range(len(tree_idx)):
            idx = tree_idx[i]
            self.memory.update(idx, (abs_errors[i] + 1e-5) ** self.alpha)  # 防止零优先级
            self.max_p = max(self.max_p, (abs_errors[i] + 1e-5) ** self.alpha)
        
        # 计算损失（含重要性采样权重）
        loss = (ISWeights * self.loss_function(q_target, q_eval)).mean()
