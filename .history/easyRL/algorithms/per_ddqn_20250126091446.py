...
class SumTree:
    ...
    def _propagate(self, idx, change):
        ...
        self.tree[parent] += change  # 更新父节点优先级
        if parent != 0:
            self._propagate(parent, change)  # 递归向上传播
    ...
    def _retrieve(self, idx, s):
        ...
        """
        left = 2 * idx + 1  # 左子节点索引
        if left >= len(self.tree):  # 如果是叶子节点
            return idx

        if s <= self.tree[left]:  # 如果s在左子树
    ...
    def add(self, p, data):
        ...
        Args:
            ...
            data: 经验数据
        """
        idx = self.write + self.capacity - 1  # 计算叶子节点索引
        self.data[self.write] = data  # 存储数据
        self.update(idx, p)  # 更新优先级
        self.write += 1  # 移动写入指针
        if self.write >= self.capacity:  # 循环缓冲区
            self.write = 0
        if self.n_entries < self.capacity:  # 更新存储数量
            self.n_entries += 1
    ...
    def get(self, s):
        ...
        Returns:
            (节点索引, 优先级值, 经验数据)
        """
        idx = self._retrieve(0, s)  # 从根节点开始查找
