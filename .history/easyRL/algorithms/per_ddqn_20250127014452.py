...
class PER_DDQN(DDQN):
    ...
    def learn(self, done):
        ...
        # Q值计算（复用父类核心逻辑）q_eval = self.eval_net(batch_state).gather(1, batch_action).squeeze()
        
        q_eval_next = self.eval_net(batch_next_state).detach()
        eval_act_next = q_eval_next.max(dim=1)[1].unsqueeze(1)
        
        q_next = self.target_net(batch_next_state).detach()
        q_target = batch_reward + ((~batch_done) * self.gamma * q_next.gather(1, eval_act_next)).squeeze()gather(1, eval_act_next).squeeze()
