...
class ActorCritic:
    ...
    def learn(self, done):
        ...
        with torch.no_grad():
            next_value = torch.zeros_like(batch_reward)  # 默认0值
            mask = ~batch_done  # 未终止的才计算下一个状态价值
            next_value[mask] = self.value_net(batch_next_state[mask]).squeeze()

        batch_TD_error = batch_reward + self.gamma * next_value - self.value_net(batch_state)
