...
class ActorCritic:
    ...
    def learn(self, done):
        ...
        # 假定 batch_action 为存储的 [tanh(raw_action)]
    # 这里需要将 batch_action 限制在 (-1+eps, 1-eps) 内
    eps = 1e-6
    ...
    dist = Normal(mu, sigma)
    new_log_prob = dist.log_prob(raw_action)
    new_log_prob -= torch.log(1 - batch_action.pow(2) + 1e-6)
    new_log_prob = new_log_prob.sum(dim=-1)

    policy_loss = - (new_log_prob * batch_TD_error.detach()).mean()
