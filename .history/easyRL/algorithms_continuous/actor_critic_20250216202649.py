...
class ActorCritic:
    def __init__(self,
        ...
        # 用于保存当前 on-policy 轨迹：(state, action, log_prob, reward, next_state, done)
        self.memory = []

    def choose_action(self, state):
        """
        注意：为了在更新时能够通过 log_prob 回传梯度，
        这里不要使用 torch.no_grad()，使采样保留计算图；
        但在实际环境交互中，为防止额外内存开销，可以只返回 action.detach() 给环境，
        而将 log_prob 保留用于后续训练。
        """
        state_tensor = torch.FloatTensor(state).unsqueeze(0).to(self.device)
        mu, sigma = self.policy_net(state_tensor)
        dist = Normal(mu, sigma)
        # 使用 rsample 保留 reparameterization 的梯度（对 Normal 分布可用）
