...
class PPO:
    ...
    def learn(self, done):
        ...
        actions = torch.tensor(np.array(self.states), dtype=torch.float32, device=self.device)
        raw_actions = torch.FloatTensor(np.array(self.raw_actions)).to(self.device)
        ...
        for _ in range(self.ppo_epochs):
            ...
            for start in range(0, len(indices), self.batch_size):
                ...
                dist = Normal(mu, sigma)
                new_log_probs = (dist.log_prob(batch_raw_actions).sum(dim=-1) -
