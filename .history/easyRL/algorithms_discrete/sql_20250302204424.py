...
class SQL(nn.Module):
    ...
    def choose_action(self, state, greedy=False):
        ...
        if greedy:
            ...
            # 添加数值稳定性检查
            q_temp = q_values / max(self.temperature, 1e-8)  # 防止除以接近零的值
            
            # 如果值过大，对所有Q值进行调整以避免数值溢出
            if torch.max(torch.abs(q_temp)) > 10.0:
                q_temp = q_temp / torch.max(torch.abs(q_temp)) * 10.0
            
            probs = F.softmax(q_temp, dim=1).cpu().numpy()[0]
            
            # 确保概率有效（无NaN，并且总和为1）
            if np.isnan(probs).any() or np.sum(probs) < 0.999:
                # 回退到均匀分布
