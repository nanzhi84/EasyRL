...
import torch
from easyRL.algorithms_discrete import DQN, DDQN, DUELING_DDQN, REINFORCE, PER_DDQN, PPO, SQL

@dataclass
class Config:
    algorithm: str = 'sql'              # 选择使用的算法
    ...
    temperature: float = 1.0           # 温度参数，用于控制探索程度
    temperature_decay: float = 0.999    # 温度衰减率
    temperature_min: float = 0.1       # 最小温度值
...
class AgentWrapper:
    def __init__(self, config, feature_dim, action_dim):
        ...
        elif config.algorithm.lower() == 'sql':
            self.agent = SQL(
                feature_dim=feature_dim,
                action_dim=action_dim,
                learning_rate=config.learning_rate,
