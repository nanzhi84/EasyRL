...
def train_agent(config):
    ...
    for i_episode in range(config.train_eps): 
        ...
        for i_step in range(config.max_steps):
            raw_action, log_prob = agent.choose_action(state)
            action = raw_action.cpu().numpy().squeeze()
            next_state, reward, done, truncated, info = env.step(action)

            agent.store_transition(state, raw_action, log_prob, reward, next_state, done)
