...
import torch
from easyRL.algorithms_continuous import ActorCritic, PPO, DDPG, TD3, TRPO, SAC

@dataclass
class Config:
    algorithm: str = 'trpo'              # 使用的强化学习算法
    learning_rate: float = 0.0003       # 学习率
    gamma: float = 0.99                 # 折扣因子
    train_eps: int = 200                # 训练回合数
    max_steps: int = 500                # 每个回合最大步数
    memory_size: int = 10000            # 经验回放缓冲区大小
    replace_target_iter: int = 100      # 目标网络更新频率
    batch_size: int = 32                # 训练批大小
    clip_epsilon: float = 0.2           # PPO算法中用于限制策略更新幅度的裁剪参数
    ppo_epochs: int = 8                 # PPO算法中每次更新时进行梯度更新的轮数
    gae_lambda: float = 0.95            # 广义优势估计(GAE)中的λ参数
    policy_noise: float = 0.2           # TD3算法中添加到目标策略的噪声标准差
    noise_clip: float = 0.05            # TD3算法中噪声裁剪范围
    tau: float = 0.005                  # SAC算法中软更新目标网络的参数
    alpha: float = 0.2                  # SAC算法中熵正则化系数
    auto_entropy_tuning: bool = True    # SAC算法中是否自动调整熵权重
    max_kl: float = 1e-2               # TRPO算法中最大KL散度
    cg_iters: int = 10                  # TRPO算法中共轭梯度迭代步数
    damping: float = 1e-1               # TRPO算法中阻尼系数
...
class AgentWrapper:
    def __init__(self, config, feature_dim, action_dim):
        ...
        elif config.algorithm.lower() == 'trpo':
            self.agent = TRPO(
                feature_dim=feature_dim,
                action_dim=action_dim,
                learning_rate=config.learning_rate,
...
def train_agent(config):
    ...
    for i_episode in range(config.train_eps): 
        ...
        for i_step in range(config.max_steps):
            if config.algorithm.lower() == 'ppo' or config.algorithm.lower() == 'trpo':
            ...
            elif config.algorithm.lower() == 'actor-critic':
...
def plot_rewards(rewards, ma_rewards, config):
    ...
    plt.title(f"{config.algorithm} on halfcheetah-v5")
    plt.xlabel('Episode')
    plt.ylabel('Reward')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{config.algorithm}_halfcheetah_v5.png")
    plt.show()
    plt.close()
